ALTER TABLE aiceo_memory_events ADD COLUMN IF NOT EXISTS append_sequence bigint;
...
UPDATE aiceo_memory_events e SET append_sequence=ranked.seq
FROM ranked WHERE e.id=ranked.id AND e.append_sequence IS NULL;
ALTER TABLE aiceo_memory_events ENABLE TRIGGER aiceo_memory_event_append_only;
ALTER TABLE aiceo_memory_events ALTER COLUMN append_sequence SET NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS aiceo_memory_event_project_sequence_unique
ON aiceo_memory_events(project_id,append_sequence);

CREATE OR REPLACE FUNCTION aiceo_memory_event_db_chain() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE h text; next_sequence bigint; canonical text;
BEGIN
  ...
  SELECT coalesce(max(append_sequence),0)+1 INTO next_sequence
    FROM aiceo_memory_events WHERE project_id=NEW.project_id;
  ...
  canonical := NEW.project_id::text || ':' || NEW.candidate_id::text || ':' ||
    NEW.event_type || ':' || NEW.actor_id || ':' || next_sequence::text || ':' ||
...
BEGIN
  ...
  END IF;
  IF NEW.lifecycle<>'candidate' OR NEW.memory_type<>NEW.cognitive_state THEN RAISE EXCEPTION 'candidate lifecycle/type-state violation'; END IF;
  IF NEW.authority_level NOT IN ('ordinary_agent','external_source')
    OR NEW.memory_type NOT IN ('observation','interpretation','hypothesis') OR NEW.truth_level<>'unverified'
  ...
  FOR e IN SELECT value FROM jsonb_array_elements(NEW.evidence_lineage) LOOP
    IF coalesce(e->>'sourceType','')='' OR coalesce(e->>'sourceId','')=''
      OR coalesce(e->>'evidenceHash','') !~ '^[a-f0-9]{64}$' OR coalesce(e->>'observedAt','')=''
    ...
    END;
    IF observed>now() OR (efrom IS NOT NULL AND efrom>now()) OR (euntil IS NOT NULL AND euntil<=now())
